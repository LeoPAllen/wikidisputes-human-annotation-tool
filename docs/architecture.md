...
Codebook evolution is independent of the dataset. Every event records `schema_version` and the entire codebook file's SHA-256. Reusing a version with a changed hash is rejected and requires a version increment. Progress, contextual dependencies, current-schema exports, and editable projections include only records matching the active version and hash; append-only event exports retain earlier-schema history. `schema_locked` is an optional administrative freeze, not a prerequisite for annotation.
