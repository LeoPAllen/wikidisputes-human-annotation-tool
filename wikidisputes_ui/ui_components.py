...
def turn_card(row: pd.Series, reason: str | None = None) -> None:
    ...
    st.markdown(
        f"<div class='turn'>{prefix}<div class='turn-meta'>#{int(row['utterance_order'])} · {html.escape(str(row['speaker_id']))} · {html.escape(str(row['timestamp']))} · {html.escape(str(row['utterance_id']))}</div>{html.escape(str(row['utterance_text'])).replace(chr(10), '<br>')}</div>",
        unsafe_allow_html=True,
    )
